...
async def check_permissions_and_call_tool(
    ...
    # 注：SessionManager 应包含 hook_state: HookState 和 permission_engine: PermissionEngine 属性
    # （见 modules.md SessionManager 定义）
    ...
    if perm_result == PermissionResult.NEEDS_PROMPT:
        yield SessionEvent(
            ...
            data=PermissionRequestData(
                ...
                risk_level=tool.risk_level if hasattr(tool, "risk_level") else "medium",
            ),
        )
    ...
    context = ToolUseContext(
        message_id=session.current_message_id,  # 注：需在 SessionManager 中定义此属性
        tool_use_id=tool_use.id,
    ...
    # [9] 格式化并 yield 结果
    # 注：ToolOutput 需要新增 is_error: bool = False 字段（见 modules.md）
    yield SessionEvent(
        type=TOOL_RESULT,
        data=ToolResult(
            tool_use_id=tool_use.id,
...
> 📖 **Skill 系统详解**：[skill-system.md](./skill-system.md) — SkillTool/SlashCommandTool 完整设计、技能发现、YAML frontmatter、contextModifier 机制、Marketplace、Plugin 架构。SkillTool/SlashCommandTool 的工具结果中携带的 `context_modifier` 字段会修改后续递归调用的 `options`，详见 [skill-system.md — contextModifier 机制](./skill-system.md#contextmodifier-机制)。

### contextModifier 集成

SkillTool 和 SlashCommandTool 的结果可能包含 `context_modifier` 字段（类型 `ContextModifier`），用于修改后续对话上下文：

- `allowed_tools`：限制后续可用的工具列表
- `model`：切换 LLM 模型
- `max_thinking_tokens`：设置思考 token 预算
- `context_modifier`：上下文修改器（详见 [skill-system.md — contextModifier 机制](./skill-system.md#contextmodifier-机制)）

`check_permissions_and_call_tool()` 在 `collect_tool_result()` 后需检查 `result.context_modifier`，若非 None 则更新 `options` 并传播到下一次 `query_core()` 递归。
...
| Auto-compact（触发逻辑框架） | **Phase 3** | 阈值检查 + 消息截断策略 |
| Auto-compact（完整实现） | **Phase 6** | LLM 摘要生成策略 |
