 ...
 *
 * Ported from Kode-Agent src/acp/jsonrpc.ts.
 * Supports request/response, notifications, timeouts, and abort signals.
 */
...
export class JsonRpcPeer {
  ...
  private async handleIncomingOne(payload: unknown): Promise<JsonRpcResponse | null> {
    ...
    const params = "params" in payload ? (payload as Record<string, unknown>).params : undefined
    const id = hasId ? (payload.id as string | number) : null

    const handler = this.handlers.get(method)
    if (!handler) {
      if (id === null) return null // notification for unknown method — ignore
  ...
  private handleResponse(msg: JsonRpcResponse): void {
    ...
    if (msg && typeof msg === "object" && "error" in msg && (msg as Record<string, unknown>).error) {
      const e = (msg as Record<string, unknown>).error as Record<string, unknown>
    ...
    pending.resolve((msg as Record<string, unknown>).result)
  }
  ...
  private sendRaw(obj: unknown): void {
    ...
    if (!send) {
      throw new Error("JsonRpcPeer send() not configured — call setSend() first")
    }
